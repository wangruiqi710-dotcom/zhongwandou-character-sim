...
export function meaningfulEntries(s,cid){
 ...
 for(const log of s.history){
  ...
  }else if((log.character_id===cid||log.kind==='player_choice'&&log.before?.character?.current_household_id===s.characters[cid]?.current_household_id)&&['player_choice','event','marriage','enroll','employ','succession','inheritance'].includes(log.kind))out.push({month:log.month,actor:log.kind==='player_choice'?'你的决定':'人物决定',domain:log.kind==='player_choice'?'玩家决定':log.result?.event?.category||log.kind,title:(log.kind==='player_choice'&&s.characters[log.character_id]?'〔'+s.characters[log.character_id].surname+s.characters[log.character_id].given_name+'〕':'')+(log.result?.player_choice||log.result?.event?.title||log.kind),result:log.result?.decision?.content_result?.outcome||log.result?.immediate_effect||log.result?.actual_changes?.join('；')||log.result?.status||'已记录状态变化',details:log.result?.decision?.content_result});
 }
 return out;
}
