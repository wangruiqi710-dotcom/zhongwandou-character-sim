import {ensureLocalContacts} from '../systems/remote_life.js';
import {locationAllows,locationOf} from '../systems/location_context.js';
...
export function contentImpact(action){
 ...
 return '提供此项安排；预计立即影响：'+(action.conditions.content_effects||[]).map(e=>(labels[e.kind]||e.kind)+(e.skill?'（'+e.skill+'）':'')+(['household','personal','transfer'].includes(e.kind)?' '+(e.value>0?'+':'')+e.value:'')).join('、')+'。仅现实硬阻碍或明确强冲突可阻止家庭安排；收益和学习可能不如预期。';
}
export function contentContext(s,cid){
 ...
 const roles={none:[null],family:kin,parent:parents,sibling,child:children.filter(x=>age(s,x)>=3&&age(s,x)<18).map(x=>x.character_id),adultkin:kin.filter(id=>age(s,s.characters[id])>=18),elder:kin.filter(id=>age(s,s.characters[id])>=55),spouse:spouse?[spouse]:[],mentor:s.characters[c.mentor_character_id]?.alive&&sameLocality(s,cid,c.mentor_character_id)?[c.mentor_character_id]:people.filter(x=>age(s,x)>=25&&active(s.careers[x.character_id])).map(x=>x.character_id),peer:people.filter(x=>active(s.careers[x.character_id])&&age(s,x)>=16).map(x=>x.character_id),neighbor:people.filter(x=>!kin.includes(x.character_id)).map(x=>x.character_id),friend:known.filter(id=>!parents.includes(id)&&id!==spouse),sick:kin.filter(id=>s.health[id].value<80),candidate:marriageCandidates(s,cid).filter(x=>!x.reason).map(x=>x.character_id)};
...
export function eligibleContent(s,cid,t,ctx=contentContext(s,cid)){
 if(!locationAllows(s,cid,t)||!ctx.roles[t.participant_requirements.role]?.some(id=>locationAllows(s,cid,t,id)))return false;
...
export function instantiateContent(s,cid,t,ctx=contentContext(s,cid)){
 let candidates=ctx.roles[t.participant_requirements.role].filter(id=>locationAllows(s,cid,t,id));
 if(!locationAllows(s,cid,t))return null;
...
export function contentFeasibility(s,cid,event,action){
 if(!locationAllows(s,cid,event))return '人物或相关参与者不在事件要求的地点';
 ...
 if(target&&!sameLocality(s,cid,target)&&effects.some(e=>e.kind==='care'||e.kind==='targetskill'||e.kind==='targethealth'&&!effects.some(x=>x.kind==='household'&&x.value<0)))return '这项现场照护或面对面指导要求实际同地；可选择远程资金支持';
...
export function applyContent(s,cid,result,marriageHandler=arrangeMarriage){
 ...
 case 'relocation':startAway(s,cid,event.destination||{});ensureLocalContacts(s,cid);employ(s,cid,v);break;
 case 'care':hh.responsibilities[cid]={type:'family_care',target_id:target,mandatory:true,until:s.current_world_month+12};break;
 case 'distance':reassign();startAway(s,cid,{context:'nearby',purpose:'family',duration:3});break;
