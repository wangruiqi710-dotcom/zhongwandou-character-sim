import {personLocationView} from './location_views.js';
...
export function importantEntry(e){return e.kind!=='month'||e.notices?.length||e.remote_followups?.length||e.births?.length||e.reports?.some(r=>r.death||r.player_opportunity?.waiting||r.decision||r.developments?.some(d=>d.kind!=='unchanged')||r.diff?.some(d=>['education.status','career.status','career.id'].includes(d.path)));}
