...
export function importantEntry(e){return e.kind!=='month'||e.births?.length||e.reports?.some(r=>r.death||r.player_opportunity?.waiting||r.decision||r.developments?.some(d=>d.kind!=='unchanged')||r.diff?.some(d=>['education.status','career.status','career.id'].includes(d.path)));}
