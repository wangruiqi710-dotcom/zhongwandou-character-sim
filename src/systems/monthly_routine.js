...
export function routineSummary(s,cid,d,before,relations,resourceChanges){
 ...
 tracks.family={members:hh.members.filter(id=>s.characters[id].alive).length};tracks.health={before:before.health.value,after:s.health[cid].value,fatigue:c.dynamic.fatigue};tracks.resources=resourceChanges.find(x=>x.household_id===c.current_household_id);
 const interactions=relations.filter(r=>r.people.includes(cid));if(interactions.length)tracks.relationships=interactions;
 if(children.length)tracks.parenting={children:children.length,young_children:children.filter(x=>age(s,x)<3).length};if(pregnancy)tracks.pregnancy={month:s.current_world_month-pregnancy.start_month+1,due_month:pregnancy.due_month};
