...
import {marriageCandidates,marriageHardConditions} from './marriage.js';
...
function rawChoices(s,e){
 ...
 if(e.type==='marriage')return marriageCandidates(s,e.character_id).filter(x=>!x.reason&&s.characters[x.character_id].sex!==c.sex).sort((a,b)=>Number(b.character_id===e.payload.event?.target_id)-Number(a.character_id===e.payload.event?.target_id)).slice(0,s.config.marriage_candidate_count).map(x=>choice(x.character_id,'重点推动与'+names(s,x.character_id)+'议亲','提供具体婚配机会；本人、对象及家庭继续独立回应',{target_id:x.character_id})).concat(choice('skip','暂缓议亲','本次不启动婚配、不支付费用；保留对象与历史'),choice('reject','拒绝本次婚配安排','结束本次家庭推动，不再对本次提议执行人物抽样'));
