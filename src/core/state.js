...
export function createState(seed=12345,world_id='ancient',config=defaults()){
 return {meta:{...META},seed:seed>>>0,rng_state:seed>>>0,next_id:0,world_id,remote_life_threads:{},current_world_month:2026*12,config:clone(config),characters:{},households:{},health:{},education:{},careers:{},relationships:{},marriages:{},reproduction:{},long_term_states:{},ongoing_situations:{},recent_history:{},marriage_proposal_history:[],player_decision_events:[],resources:{households:{},personal:{}},control:{current_control_character_id:null,pending:null,ended:false},inheritances:{},player:{window_last_month:2026*12,pending:null,force_credits:0,fulfilled_wishes:0,directions:['学习','家庭','健康']},history:[]};
}
export function snapshot(s,cid){const c=s.characters[cid],hid=c?.current_household_id;return {character:clone(c),character_age:c?age(s,c):null,household:clone(s.households[hid]||null),remote_life_threads:clone(Object.values(s.remote_life_threads||{}).filter(t=>t.character_id===cid)),health:clone(s.health[cid]||null),education:clone(s.education[cid]||null),career:clone(s.careers[cid]||null),resources:{household:clone(s.resources.households[hid]||null),personal:clone(s.resources.personal[cid]||null)},relationships:clone(Object.values(s.relationships).filter(r=>r.people.includes(cid))),marriages:clone(Object.values(s.marriages).filter(r=>r.people.includes(cid))),ongoing_situations:clone(Object.values(s.ongoing_situations).filter(x=>x.participants.includes(cid))),recent_history:clone(s.recent_history[cid]||[]),long_term_states:clone(Object.values(s.long_term_states).filter(r=>r.character_id===cid))};}
