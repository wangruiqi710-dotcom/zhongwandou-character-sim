...
export const canContinue=run=>run?.mock_version===META.mock_version&&run.schema_version===META.schema_version&&!!run.state.remote_life_threads&&Object.values(run.state.characters).every(c=>c.current_location_context&&c.location_context_ref);
export function step(run,cmd){if(!canContinue(run))throw Error('旧版本存档仅可查看和导出，请新建本版运行；未静默修改旧数据');const backup=clone({...run.state,history:[]});backup.history=[...run.state.history];let entry;try{entry=command(run.state,cmd);}catch(e){run.state=backup;throw e;}run.commands.push(clone(cmd));entry.command_index=run.commands.length-1;return entry;}
export function replay(run,index=run.commands.length-1){if(!canContinue({...run,state:run.initial_state})||run.design_sha256!==META.design_sha256)throw Error('该记录来自旧 Mock 版本或其他设计版本');const s=clone(run.initial_state);for(let i=0;i<=index;i++)command(s,run.commands[i]);return s;}
