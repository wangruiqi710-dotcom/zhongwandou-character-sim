...
(async()=>{
 ...
 const {resolveDecision,decide}=await import('../src/systems/behavior.js'),{command,month}=await import('../src/core/simulation.js'),{pendingDecision,playerChoices,needsPlayer}=await import('../src/systems/player_decisions.js');
 ...
 report.audit.player=ANCIENT_EVENTS.filter(t=>t.importance==='major').length;report.audit.multistage=ANCIENT_EVENTS.filter(t=>t.transition_actions.length).length;report.audit.real_npc=ANCIENT_EVENTS.filter(t=>t.participant_requirements.real_person).length;report.audit.long_term=ANCIENT_EVENTS.filter(t=>t.developmental).length;report.audit.ongoing=ANCIENT_EVENTS.filter(t=>t.possible_ongoing_situations.length).length;report.audit.developmental=ANCIENT_EVENTS.filter(t=>t.developmental).length;
