async function reveal(locator){await locator.evaluate(el=>{for(let p=el.parentElement;p;p=p.parentElement)if(p.tagName==='DETAILS')p.open=true;});return locator;}
...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});try{
 ...
 await page.locator('#new-life').click();await page.locator('#summary h3').first().waitFor();await page.locator('[data-months="1"]').click();await page.getByText('已逐月运行 1个月',{exact:true}).waitFor();
 ...
 await (await reveal(page.locator('#timeline summary').filter({hasText:/查看原因/}).first())).click();
 assert.equal(await page.locator('#timeline pre:visible').count(),0);
 await (await reveal(page.locator('[data-rate="合理"]').last())).click();await page.getByText('已保存“合理”及完整现场').waitFor();
 await (await reveal(page.locator('[data-rate="问题"]').last())).click();await page.locator('#issue-type').selectOption({label:'现实条件判断有问题'});await page.locator('#save-issue').click();
 ...
 await page.locator('nav [data-page="simulation"]').click();await (await reveal(page.locator('#summary [data-person]').first())).click();await page.getByText('查看完整人物',{exact:true}).waitFor();await page.locator('[data-return-person]').first().click();
 ...
 await page.locator('#choice-dialog [data-person]').first().click();await page.locator('[data-return-person]').first().click();
 ...
 await page.locator('nav [data-page="tests"]').click();await page.locator('#seed').fill('31');await page.locator('#new-life').click();await (await reveal(page.locator('[data-months="120"]'))).click();await page.locator('#choice-dialog[open]').waitFor({timeout:30000});
