...
(async()=>{
 ...
 const {startAway}=await import('../src/systems/life_context.js'),{joinHousehold}=await import('../src/systems/household.js');
 const tests=[],audit=[];
 ...
 const withoutOwner=t=>{const {event_type,player_involvement,flow_type,decision_audit,location_scope,location_requirement,...rest}=t;return rest;};assert.deepEqual(ANCIENT_EVENTS.map(withoutOwner),baseline.ANCIENT_EVENTS.map(withoutOwner));
 for(const file of ['decisions.js','worlds.js','src/core/rng.js','src/systems/genetics.js','src/systems/ongoing_situations.js','src/systems/decision_reasons.js','src/systems/player_intent.js','src/systems/death.js','src/systems/succession_inheritance.js'])assert.equal(fs.readFileSync(path.join(root,file),'utf8').replace(/\r/g,''),execFileSync('git',['show','860fa180646252abd9b7e9dfb9771ac25829e468:'+file],{cwd:root,encoding:'utf8'}).replace(/\r/g,''),'protected core changed '+file);
 tests.push('PROTECTED-CORE: content unchanged except ownership/location requirements; probability/RNG/world/genetics/reasons/intervention/death/inheritance source unchanged');
 ...
 for(const t of ANCIENT_EVENTS){
  const s=fixture({seed:5}),cid=s.test.child_id;if(t.location_scope==='remote_only')startAway(s,cid);const ctx=contentContext(s,cid);
  ...
  const role=t.participant_requirements.role;const rid=ctx.roles[role][0];if(t.location_scope==='same_household'&&rid)joinHousehold(s,rid,s.characters[cid].current_household_id);if(role==='peer')s.careers[rid]={...s.careers[cid]};if(role==='mentor'){s.characters[rid].birth_month=s.current_world_month-40*12;s.characters[rid].skills.push({name:s.education[cid]?.skill||s.careers[cid]?.skill||t.interest_examples[0],level:40});}if(t.trigger_conditions.condition==='seriouskin')s.health[rid].value=35;if(t.trigger_conditions.condition==='schoolchild')s.education[rid]={status:'active'};const e=instantiateContent(s,cid,t,ctx);assert(e,t.event_id);const before=facts(s),type=playerDecisionType(e),gate=needsPlayer(s,cid,e);assert.equal(t.event_type==='PLAYER_DECISION_EVENT',!!type);assert.equal(t.flow_type!=='ROUTINE_MONTHLY_PROGRESS'&&t.player_involvement!=='人物自主',!!type);
